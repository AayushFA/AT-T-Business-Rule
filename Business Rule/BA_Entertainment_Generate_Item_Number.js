...
{
  "id" : "BA_Entertainment_Generate_Item_Number",
  "type" : "BusinessAction",
  "setupGroups" : [ "Entertainment_Item_Number_Generation" ],
  "name" : "Entertainment Generate Item Number",
  "description" : null,
  "scope" : "Global",
...
retailEntertainmentItemNumberGeneration(node, step)
 //(PITEM-7540947)
//checkAndSetEnteredValueItemNumber(node, step)
        ...
        if (comSku.getValue("Item_Num").getSimpleValue() == null || comSku.getValue("Item_Num").getSimpleValue() == "") {
          ...
          if (parLob == "RTL") {
            ...
            if (compType.trim().length() == 1) {
              ...
              newCompItemNo = initial.trim() + "." + returnInitials(comSku, step) + compItemNo.trim().slice(1) + compType;
      ...
      child.forEach(function(compSKU) {
        ...
        if (objectType == "Companion_SKU") {
          if (compSKU.getValue("Item_Num").getSimpleValue() == null || compSKU.getValue("Item_Num").getSimpleValue() == "") {
            ...
            if (parentItemClass == "BAU_Broadband") {
              var newItemNumberBase = replaceFirstWithR(itemNumberBase);
              if (compSKU.getValue("Item_Num").getSimpleValue() == null || compSKU.getValue("Item_Num").getSimpleValue() == "") {
