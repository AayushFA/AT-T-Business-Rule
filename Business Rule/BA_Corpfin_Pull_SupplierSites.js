...
{
  "id" : "BA_Corpfin_Pull_SupplierSites",
  "type" : "BusinessAction",
  "setupGroups" : [ "Supplier_BA" ],
  "name" : "Corpfin Pull SupplierSites",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
*/
exports.operation0 = function (step,giep,lookuptable,log,entityObject,mailHome,globalLib,auditLib) {
log.severe("Start Of BA_Corpfin_Pull_SupplierSites");
...
var subject = "Supplier Sites Bulk-Update Scheduled Process Failure Notification";
...
while (itemsCounter) {
 try{
  var response = post.path("/SupplierSites/fetch").pathQuery({
    ...
    Object.keys(responseData).forEach(function(key) {
      if (key == "items") {
        ...
        if (responseData[key].length > 0) {
          pageNumber++;
          itemsCounter = true;
          responseData[key].forEach(function(supplierSite) {
           try{
            processSupplierSite(supplierSite, log, lookuptable, step);
           }catch (processError){
           	if(processError){		          	
...
log.severe("End Of BA_Corpfin_Pull_SupplierSites");

function processSupplierSite(supplierSite, log,lookuptable,step){
	var supNo = "";
	var supSiteCode = "";
     var supplierAttsArray = globalLib.getStiboAttsForSupplierMap(lookuptable,"supplierAtts");
    var supplierSiteAttsArray = globalLib.getStiboAttsForSupplierMap(lookuptable,"supplierSiteAtts");
    Object.keys(supplierSite).forEach(function(key) {
        if(key == "SUPPLIER_NUMBER"){
		     supNo = supplierSite[key];			     
        }
	   if(key == "SUPPLIER_SITE_CODE"){
	         supSiteCode = supplierSite[key];	    
	   }
    });		    							
			var supplierObj = step.getNodeHome().getObjectByKey("Supplier_Key", supNo);			
			var supplierSiteObj = step.getNodeHome().getObjectByKey("SupplierSite_Key", supNo+"_"+supSiteCode);
			if(supplierSiteObj){				
			   globalLib.updateSupplierAttrs(supplierSiteAttsArray, log, lookuptable, supplierSite, supplierSiteObj,step);			   
                 auditLib.setDateTime(supplierSiteObj, "Last_Updated_DateTime");		   
			}
			else{			  
			   if(supplierObj)	{		   			    			   	
			      supplierSiteObj = globalLib.createSupplierSite(supplierObj,supNo,supSiteCode,auditLib);
                    globalLib.updateSupplierAttrs(supplierSiteAttsArray, log, lookuptable, supplierSite, supplierSiteObj,step);                      	                  	
			   }
			   else{
			   	 supplierObj = globalLib.createSupplier(supNo,step,auditLib);	
			   	 globalLib.updateSupplierAttrs(supplierAttsArray, log, lookuptable, supplierSite, supplierObj,step);
			   	 supplierObj.approve();
			   	 supplierSiteObj = globalLib.createSupplierSite(supplierObj,supNo,supSiteCode,auditLib);
			   	 globalLib.updateSupplierAttrs(supplierSiteAttsArray, log, lookuptable, supplierSite, supplierSiteObj,step);				   	 				   	 		
			   }
			}	
			supplierSiteObj.approve();				
}
